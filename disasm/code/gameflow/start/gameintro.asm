
; ASM FILE code\gameflow\start\gameintro.asm :
; 0x130004..0x131A1C : Game intro

; =============== S U B R O U T I N E =======================================

PlayIntro:
		jmp     *+4(pc)
		jsr     (j_DisableDisplayAndInterrupts).l
		bsr.w   InitDisplay_Intro
		clr.b   ((MESSAGE_SPEED-$1000000)).w
		movea.l (p_palette_IntroScene1).l,a0
		lea     (PALETTE_1_BASE).l,a1
		move.w  #$80,d7 
		jsr     (j_CopyBytes).l
		movea.l (p_tiles_IntroSpeckOfLight).l,a0
		lea     ($2000).w,a1
		move.w  #$1000,d0
		move.w  #2,d1
		jsr     (j_ApplyImmediateVramDmaOnCompressedTiles).l
		openTxt
		move.b  #2,((byte_FFF804-$1000000)).w
		move.w  #5,((word_FFB7C6-$1000000)).w
		clr.w   ((CURRENT_OBJECT-$1000000)).w
		jsr     (j_EnableDisplayAndInterrupts).l
		move.l  #sub_1305B4,(VINT_CONTEXTUAL_FUNCTION_ADDRESS).l
		jsr     (j_FadeInFromBlack).l
		txt     82              ; "In ages long forgotten..."
		move.w  #180,d0 
		bsr.w   CheckSkipToTitleScreen
		clearTxt
		txt     83              ; "...Light fought Darkness for[Line]control of the world."
		move.w  #180,d0 
		bsr.w   CheckSkipToTitleScreen
		clearTxt
		txt     84              ; "Dark Dragon led the evil[Line]hordes of Darkness."
		move.w  #180,d0 
		bsr.w   CheckSkipToTitleScreen
		closeTxt
		move.w  #5,((word_FFB7C6-$1000000)).w
		move.w  #$10,((CURRENT_OBJECT-$1000000)).w
		move.w  #25,d0
		bsr.w   CheckSkipToTitleScreen
		clr.l   (VINT_CONTEXTUAL_FUNCTION_ADDRESS).l
		jsr     (j_FadeOutToWhite).l
		lea     (PALETTE_1_BASE).l,a0
		moveq   #31,d7
loc_1300C4:
		move.l  #$EEE0EEE,(a0)+
		dbf     d7,loc_1300C4
                
		clr.l   (VINT_CONTEXTUAL_FUNCTION_ADDRESS).l
		jsr     (j_FadeOutToBlack).l
		jsr     (j_DisableDisplayAndInterrupts).l
		bsr.w   InitDisplay_Intro
		bsr.w   sub_1316A8
		movea.l (p_tiles_IntroScene1_1).l,a0
		lea     ($2000).w,a1
		move.w  #$C00,d0
		move.w  #2,d1
		jsr     (j_ApplyImmediateVramDmaOnCompressedTiles).l
		movea.l (p_tiles_IntroScene1_2).l,a0
		lea     ($3800).w,a1
		move.w  #$C00,d0
		move.w  #2,d1
		jsr     (j_ApplyImmediateVramDmaOnCompressedTiles).l
		movea.l (p_palette_IntroScene1).l,a0
		lea     (PALETTE_1_BASE).l,a1
		move.w  #$80,d7 
		jsr     (j_CopyBytes).l
		openTxt
		move.b  #2,((byte_FFF804-$1000000)).w
		jsr     (j_EnableDisplayAndInterrupts).l
		jsr     (j_FadeInFromBlack).l
		txt     85              ; "The Ancients fought back with[Line]the Powers of Light."
		move.w  #180,d0 
		bsr.w   CheckSkipToTitleScreen
		clearTxt
		txt     86              ; "Dark Dragon was defeated and[Line]cast into another dimension."
		move.w  #360,d0
		bsr.w   CheckSkipToTitleScreen
		closeTxt
		jsr     (j_FadeOutToBlack).l
		jsr     (j_DisableDisplayAndInterrupts).l
		bsr.w   InitDisplay_Intro
		movea.l (p_palette_IntroScene2).l,a0
		lea     (PALETTE_1_BASE).l,a1
		move.w  #$20,d7 
		jsr     (j_CopyBytes).l
		movea.l (p_tiles_IntroScene2).l,a0
		lea     ($2000).w,a1
		move.w  #$1000,d0
		move.w  #2,d1
		jsr     (j_ApplyImmediateVramDmaOnCompressedTiles).l
		openTxt
		move.b  #2,((byte_FFF804-$1000000)).w
		move.w  #5,((word_FFB7C6-$1000000)).w
		clr.w   ((CURRENT_OBJECT-$1000000)).w
		jsr     (j_EnableDisplayAndInterrupts).l
		move.l  #sub_13062A,(VINT_CONTEXTUAL_FUNCTION_ADDRESS).l
		jsr     (j_FadeInFromBlack).l
		txt     87              ; "The Lord of Darkness vowed to[Line]return in 1,000 years."
		move.w  #360,d0
		bsr.w   CheckSkipToTitleScreen
		closeTxt
		move.w  #25,d0
		bsr.w   CheckSkipToTitleScreen
		clr.l   (VINT_CONTEXTUAL_FUNCTION_ADDRESS).l
		jsr     (j_FadeOutToWhite).l
		lea     (PALETTE_1_BASE).l,a0
		moveq   #31,d7
loc_1301F4:
		move.l  #$EEE0EEE,(a0)+
		dbf     d7,loc_1301F4
                
		clr.l   (VINT_CONTEXTUAL_FUNCTION_ADDRESS).l
		jsr     (j_FadeOutToBlack).l
		jsr     (j_DisableDisplayAndInterrupts).l
		bsr.w   InitDisplay_Intro
		bsr.w   sub_1316A8
		movea.l (p_tiles_IntroScene3_1).l,a0
		lea     ($2000).w,a1
		move.w  #$C00,d0
		move.w  #2,d1
		jsr     (j_ApplyImmediateVramDmaOnCompressedTiles).l
		movea.l (p_tiles_IntroScene3_2).l,a0
		lea     ($3800).w,a1
		move.w  #$C00,d0
		move.w  #2,d1
		jsr     (j_ApplyImmediateVramDmaOnCompressedTiles).l
		movea.l (p_palette_IntroScene1).l,a0
		lea     (PALETTE_1_BASE).l,a1
		move.w  #$80,d7 
		jsr     (j_CopyBytes).l
		movea.l (p_palette_IntroScene3).l,a0
		lea     (PALETTE_1_BASE).l,a1
		move.w  #$20,d7 
		jsr     (j_CopyBytes).l
		openTxt
		move.b  #2,((byte_FFF804-$1000000)).w
		jsr     (j_EnableDisplayAndInterrupts).l
		jsr     (j_FadeInFromBlack).l
		txt     88              ; "Time passed, and Dark Dragon[Line]was forgotten by all."
		move.w  #180,d0 
		bsr.w   CheckSkipToTitleScreen
		clearTxt
		txt     89              ; "Ten centuries of peace ruled[Line]the land of Rune."
		move.w  #360,d0
		bsr.w   CheckSkipToTitleScreen
		closeTxt
		jsr     (j_FadeOutToBlack).l
		jsr     (j_DisableDisplayAndInterrupts).l
		bsr.w   InitDisplay_Intro
		bsr.w   sub_1316D8
		movea.l (p_tiles_IntroScene4_1).l,a0
		lea     ($2000).w,a1
		move.w  #$C00,d0
		move.w  #2,d1
		jsr     (j_ApplyImmediateVramDmaOnCompressedTiles).l
		movea.l (p_tiles_IntroScene4_2).l,a0
		lea     ($3800).w,a1
		move.w  #$C00,d0
		move.w  #2,d1
		jsr     (j_ApplyImmediateVramDmaOnCompressedTiles).l
		movea.l (p_tiles_IntroScene4_BG1).l,a0
		lea     ($6000).w,a1
		move.w  #$C00,d0
		move.w  #2,d1
		jsr     (j_ApplyImmediateVramDmaOnCompressedTiles).l
		movea.l (p_tiles_IntroScene4_BG2).l,a0
		lea     ($7800).w,a1
		move.w  #$C00,d0
		move.w  #2,d1
		jsr     (j_ApplyImmediateVramDmaOnCompressedTiles).l
		movea.l (p_palette_IntroScene4).l,a0
		lea     (PALETTE_1_BASE).l,a1
		move.w  #$20,d7 
		jsr     (j_CopyBytes).l
		openTxt
		move.b  #2,((byte_FFF804-$1000000)).w
		move.w  #$1E,((word_FFB7C6-$1000000)).w
		clr.w   ((CURRENT_OBJECT-$1000000)).w
		jsr     (j_EnableDisplayAndInterrupts).l
		move.l  #sub_130694,(VINT_CONTEXTUAL_FUNCTION_ADDRESS).l
		jsr     (j_FadeInFromBlack).l
		txt     90              ; "Until the kingdom of Runefaust[Line]brought war and fear to Rune."
		move.w  #180,d0 
		bsr.w   CheckSkipToTitleScreen
		clearTxt
		txt     91              ; "Hordes of evil creatures[Line]ravaged every land."
		move.w  #180,d0 
		bsr.w   CheckSkipToTitleScreen
		clearTxt
		txt     92              ; "Here and there, strongholds[Line]of Good still held out..."
		move.w  #180,d0 
		bsr.w   CheckSkipToTitleScreen
		clearTxt
		txt     93              ; "...awaiting a Hero who could[Line]wield the Powers of Light!"
		move.w  #360,d0
		bsr.w   CheckSkipToTitleScreen
		closeTxt
		clr.l   (VINT_CONTEXTUAL_FUNCTION_ADDRESS).l
		sndCom  SOUND_COMMAND_FADE_OUT
		jsr     (j_FadeOutToBlack).l
		move.w  #60,d0 
		bsr.w   CheckSkipToTitleScreen
		sndCom  MUSIC_TITLE
		move.w  #90,d0 
		bsr.w   CheckSkipToTitleScreen
		move.w  #$280,((CURRENT_OBJECT-$1000000)).w
TitleScreen:
		jsr     (j_DisableDisplayAndInterrupts).l
		bsr.w   InitDisplay_Intro
		movea.l (p_layout_TitleScreen).l,a0
		lea     (PLANE_A_MAP_LAYOUT).l,a1
		move.w  #$700,d7
		jsr     (j_CopyBytes).l
		movea.l (p_layout_TitleScreen).l,a0
		lea     ($E000).l,a1
		move.w  #$380,d0
		move.w  #2,d1
		jsr     (j_ApplyImmediateVramDma).l
		movea.l (p_palette_TitleScreen).l,a0
		lea     (PALETTE_1_BASE).l,a1
		move.w  #$20,d7 
		jsr     (j_CopyBytes).l
		movea.l (p_tiles_TitleScreen).l,a0
		lea     ($2000).w,a1
		move.w  #$1000,d0
		move.w  #2,d1
		jsr     (j_ApplyImmediateVramDmaOnCompressedTiles).l
		jsr     (j_EnableDisplayAndInterrupts).l
		jsr     (j_FadeInFromBlack).l
		move.l  #sub_130706,(VINT_CONTEXTUAL_FUNCTION_ADDRESS).l
		move.w  #$5A0,d0
		bsr.w   sub_130536
		jsr     (j_FadeOutToBlack).l
		jsr     (j_DisableDisplayAndInterrupts).l
		movea.w #$2700,sp
		movea.l (InitStack).w,sp
		movea.l (p_Start).w,a0  
		jmp     (a0)

    ; End of function PlayIntro

layout_PressStartButton:
		vdpTile ASCII_UPPERCASE_P
		vdpTile ASCII_UPPERCASE_R
		vdpTile ASCII_UPPERCASE_E
		vdpTile ASCII_UPPERCASE_S
		vdpTile ASCII_UPPERCASE_S
		vdpTile 
		vdpTile ASCII_UPPERCASE_S
		vdpTile ASCII_UPPERCASE_T
		vdpTile ASCII_UPPERCASE_A
		vdpTile ASCII_UPPERCASE_R
		vdpTile ASCII_UPPERCASE_T
		vdpTile 
		vdpTile ASCII_UPPERCASE_B
		vdpTile ASCII_UPPERCASE_U
		vdpTile ASCII_UPPERCASE_T
		vdpTile ASCII_UPPERCASE_T
		vdpTile ASCII_UPPERCASE_O
		vdpTile ASCII_UPPERCASE_N
                
		jsr     (j_DisableDisplayAndInterrupts).l
		bsr.w   InitDisplay_Intro
		movea.l (p_layout_TitleScreen).l,a0
		lea     ($E000).l,a1
		move.w  #$380,d0
		move.w  #2,d1
		jsr     (j_ApplyImmediateVramDma).l
		movea.l (p_palette_TitleScreen).l,a0
		lea     (PALETTE_1_BASE).l,a1
		move.w  #$20,d7 
		jsr     (j_CopyBytes).l
		movea.l (p_tiles_TitleScreen).l,a0
		lea     ($2000).w,a1
		move.w  #$1000,d0
		move.w  #2,d1
		jsr     (j_ApplyImmediateVramDmaOnCompressedTiles).l
		jsr     (j_EnableDisplayAndInterrupts).l
		jsr     (j_FadeInFromBlack).l
		move.w  #50,d0
		jsr     (j_Sleep).l
		jsr     (j_FadeOutToBlack).l
		jsr     (j_DisableDisplayAndInterrupts).l
		rts

; =============== S U B R O U T I N E =======================================

; Wait D0 frames while checking for start button input

CheckSkipToTitleScreen:
		
		btst    #INPUT_BIT_START,(P1_INPUT).l
		beq.s   loc_13052A
		clr.l   (VINT_CONTEXTUAL_FUNCTION_ADDRESS).l
		sndCom  SOUND_COMMAND_FADE_OUT
		jsr     (j_FadeOutToBlack).l
		movem.l (sp)+,d7
		sndCom  MUSIC_TITLE
		move.w  #$50,((CURRENT_OBJECT-$1000000)).w 
		bra.w   TitleScreen
loc_13052A:
		jsr     (j_WaitForVInt).l
		dbf     d0,CheckSkipToTitleScreen
                
		rts

    ; End of function CheckSkipToTitleScreen


; =============== S U B R O U T I N E =======================================

sub_130536:
		btst    #INPUT_BIT_START,(P1_INPUT).l
		beq.s   loc_130556
		clr.l   (VINT_CONTEXTUAL_FUNCTION_ADDRESS).l
		sndCom  SOUND_COMMAND_FADE_OUT
		jsr     (j_FadeOutToBlack).l
		movem.l (sp)+,d7
		rts
loc_130556:
		jsr     (j_WaitForVInt).l
		dbf     d0,sub_130536
                
		rts

    ; End of function sub_130536

byte_130562:    dc.b 0
		dc.b 1
		dc.b 2
		dc.b 3
		dc.b 4
		dc.b 5
		dc.b 6
		dc.b 7
		dc.b 6
		dc.b 5
		dc.b 4
		dc.b 3
		dc.b 2
		dc.b 1
		dc.b $FF
		dc.b $FF
		dc.b 8
		dc.b 9
		dc.b $A
		dc.b $B
		dc.b $C
		dc.b $C
		dc.b $C
		dc.b $C
		dc.b $C
		dc.b $C
		dc.b $C
		dc.b $C
		dc.b $C
		dc.b $C
off_130580:     dc.l byte_130740
		dc.l byte_130762
		dc.l byte_130784
		dc.l byte_1307A6
		dc.l byte_1307C8
		dc.l byte_1307EA
		dc.l byte_13080C
		dc.l byte_13082E
		dc.l byte_130850
		dc.l byte_13089A
		dc.l byte_13091C
		dc.l byte_130A3E
		dc.l byte_130C40

; =============== S U B R O U T I N E =======================================

sub_1305B4:
		subq.w  #1,((word_FFB7C6-$1000000)).w
		bne.s   return_1305F6
		move.w  #5,((word_FFB7C6-$1000000)).w
loc_1305C0:
		move.w  ((CURRENT_OBJECT-$1000000)).w,d0
		move.b  byte_130562(pc,d0.w),d0
		bpl.s   loc_1305D2
		subi.w  #$E,((CURRENT_OBJECT-$1000000)).w
		bra.s   loc_1305C0
loc_1305D2:
		addq.w  #1,((CURRENT_OBJECT-$1000000)).w
		lsl.w   #2,d0
		movea.l off_130580(pc,d0.w),a0
		move.w  (a0)+,d0
		move.w  d0,d2
		lsr.w   #1,d2
		andi.w  #$3F3F,d2
		move.w  #$100C,d1
		sub.w   d2,d1
		jsr     j_CreateWindow
		bsr.w   sub_13066E
return_1305F6:
		
		rts

    ; End of function sub_1305B4

byte_1305F8:    dc.b 0
		dc.b 3
		dc.b 0
		dc.b 3
		dc.b 0
		dc.b 3
		dc.b 0
		dc.b 3
		dc.b 1
		dc.b 4
		dc.b 2
		dc.b 5
		dc.b 1
		dc.b 4
		dc.b 2
		dc.b 5
		dc.b 1
		dc.b 4
		dc.b 2
		dc.b 5
		dc.b 1
		dc.b 4
		dc.b 2
		dc.b 5
		dc.b $FF
		dc.b $FF
off_130612:     dc.l byte_13100A
		dc.l byte_131110
		dc.l byte_131216
		dc.l byte_13131C
		dc.l byte_131422
		dc.l byte_131528

; =============== S U B R O U T I N E =======================================

sub_13062A:
		subq.w  #1,((word_FFB7C6-$1000000)).w
		bne.s   return_13066C
		move.w  #$F,((word_FFB7C6-$1000000)).w
loc_130636:
		move.w  ((CURRENT_OBJECT-$1000000)).w,d0
		move.b  byte_1305F8(pc,d0.w),d0
		bpl.s   loc_130648
		subi.w  #$10,((CURRENT_OBJECT-$1000000)).w
		bra.s   loc_130636
loc_130648:
		addq.w  #1,((CURRENT_OBJECT-$1000000)).w
		lsl.w   #2,d0
		movea.l off_130612(pc,d0.w),a0
		move.w  (a0)+,d0
		move.w  d0,d2
		lsr.w   #1,d2
		andi.w  #$3F3F,d2
		move.w  #$100A,d1
		sub.w   d2,d1
		jsr     j_CreateWindow
		bsr.w   sub_13066E
return_13066C:
		
		rts

    ; End of function sub_13062A


; =============== S U B R O U T I N E =======================================

sub_13066E:
		lea     ($E000).l,a1
		move.w  #$780,d0
		lea     (PLANE_A_MAP_LAYOUT).l,a0
		moveq   #2,d1
		jsr     (j_ApplyVIntVramDma).l
		jmp     (j_EnableDmaQueueProcessing).l

    ; End of function sub_13066E

byte_13068C:    dc.b 0
		dc.b 1
		dc.b 2
		dc.b 1
		dc.b 0
		dc.b $FF
		dc.b $FF
		dc.b $FF

; =============== S U B R O U T I N E =======================================

sub_130694:
		lea     (word_FF031E).l,a0
		moveq   #3,d6
		jsr     (j_GenerateRandomNumber).l
		subq.w  #1,d7
		add.w   d7,(a0)
		lea     (dword_FF01A0).l,a0
		move.w  #$5F,d7 
loc_1306B0:
		move.l  4(a0),(a0)+
		dbf     d7,loc_1306B0
                
		jsr     (j_UpdateVdpHScrollData).l
		jsr     (j_EnableDmaQueueProcessing).l
		subq.w  #1,((word_FFB7C6-$1000000)).w
		bne.s   return_130704
		move.w  #$19,((word_FFB7C6-$1000000)).w
loc_1306D0:
		move.w  ((CURRENT_OBJECT-$1000000)).w,d0
		move.b  byte_13068C(pc,d0.w),d0
		bpl.s   loc_1306E0
		subq.w  #4,((CURRENT_OBJECT-$1000000)).w
		bra.s   loc_1306D0
loc_1306E0:
		addq.w  #1,((CURRENT_OBJECT-$1000000)).w
		movea.l (p_palette_IntroScene4).l,a0
		lsl.w   #5,d0
		adda.w  d0,a0
		lea     (PALETTE_1_CURRENT).l,a1
		move.w  #$20,d7 
		jsr     (j_CopyBytes).l
		jsr     (j_ApplyVIntCramDma).l
return_130704:
		
		rts

    ; End of function sub_130694


; =============== S U B R O U T I N E =======================================

sub_130706:
		subq.w  #1,((CURRENT_OBJECT-$1000000)).w
		move.w  ((CURRENT_OBJECT-$1000000)).w,d0
		bne.s   loc_130722
		jsr     sub_80DC
		bsr.w   sub_13066E
		move.w  #$28,((CURRENT_OBJECT-$1000000)).w 
		rts
loc_130722:
		cmpi.w  #$14,d0
		bne.s   return_13073E
		lea     layout_PressStartButton(pc), a0
		move.w  #$1201,d0
		move.w  #$711,d1
		jsr     j_CreateWindow
		bsr.w   sub_13066E
return_13073E:
		
		rts

    ; End of function sub_130706

byte_130740:    dc.b 4
		dc.b   4
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   9
		dc.b   0
		dc.b   9
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   1
		dc.b   9
		dc.b   1
		dc.b   9
		dc.b   0
		dc.b $11
		dc.b   0
		dc.b $11
		dc.b   1
		dc.b $19
		dc.b   1
		dc.b $19
		dc.b   0
		dc.b $11
		dc.b   0
		dc.b $11
		dc.b   0
		dc.b $19
		dc.b   0
		dc.b $19
		dc.b   0
byte_130762:    dc.b 4
		dc.b   4
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   9
		dc.b   0
		dc.b   9
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   2
		dc.b   9
		dc.b   2
		dc.b   9
		dc.b   0
		dc.b $11
		dc.b   0
		dc.b $11
		dc.b   2
		dc.b $19
		dc.b   2
		dc.b $19
		dc.b   0
		dc.b $11
		dc.b   0
		dc.b $11
		dc.b   0
		dc.b $19
		dc.b   0
		dc.b $19
		dc.b   0
byte_130784:    dc.b 4
		dc.b   4
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   9
		dc.b   0
		dc.b   9
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   3
		dc.b   9
		dc.b   3
		dc.b   9
		dc.b   0
		dc.b $11
		dc.b   0
		dc.b $11
		dc.b   3
		dc.b $19
		dc.b   3
		dc.b $19
		dc.b   0
		dc.b $11
		dc.b   0
		dc.b $11
		dc.b   0
		dc.b $19
		dc.b   0
		dc.b $19
		dc.b   0
byte_1307A6:    dc.b 4
		dc.b   4
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   5
		dc.b   9
		dc.b   5
		dc.b   9
		dc.b   0
		dc.b   1
		dc.b   4
		dc.b   1
		dc.b   6
		dc.b   9
		dc.b   6
		dc.b   9
		dc.b   4
		dc.b $11
		dc.b   4
		dc.b $11
		dc.b   6
		dc.b $19
		dc.b   6
		dc.b $19
		dc.b   4
		dc.b $11
		dc.b   0
		dc.b $11
		dc.b   5
		dc.b $19
		dc.b   5
		dc.b $19
		dc.b   0
byte_1307C8:    dc.b 4
		dc.b   4
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   8
		dc.b   9
		dc.b   8
		dc.b   9
		dc.b   0
		dc.b   1
		dc.b   7
		dc.b   1
		dc.b   9
		dc.b   9
		dc.b   9
		dc.b   9
		dc.b   7
		dc.b $11
		dc.b   7
		dc.b $11
		dc.b   9
		dc.b $19
		dc.b   9
		dc.b $19
		dc.b   7
		dc.b $11
		dc.b   0
		dc.b $11
		dc.b   8
		dc.b $19
		dc.b   8
		dc.b $19
		dc.b   0
byte_1307EA:    dc.b 4
		dc.b   4
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b  $B
		dc.b   9
		dc.b  $B
		dc.b   9
		dc.b   0
		dc.b   1
		dc.b  $A
		dc.b   1
		dc.b  $C
		dc.b   9
		dc.b  $C
		dc.b   9
		dc.b  $A
		dc.b $11
		dc.b  $A
		dc.b $11
		dc.b  $C
		dc.b $19
		dc.b  $C
		dc.b $19
		dc.b  $A
		dc.b $11
		dc.b   0
		dc.b $11
		dc.b  $B
		dc.b $19
		dc.b  $B
		dc.b $19
		dc.b   0
byte_13080C:    dc.b 4
		dc.b   4
		dc.b   1
		dc.b  $D
		dc.b   1
		dc.b  $F
		dc.b   9
		dc.b  $F
		dc.b   9
		dc.b  $D
		dc.b   1
		dc.b  $E
		dc.b   1
		dc.b $10
		dc.b   9
		dc.b $10
		dc.b   9
		dc.b  $E
		dc.b $11
		dc.b  $E
		dc.b $11
		dc.b $10
		dc.b $19
		dc.b $10
		dc.b $19
		dc.b  $E
		dc.b $11
		dc.b  $D
		dc.b $11
		dc.b  $F
		dc.b $19
		dc.b  $F
		dc.b $19
		dc.b  $D
byte_13082E:    dc.b 4
		dc.b   4
		dc.b   1
		dc.b $11
		dc.b   1
		dc.b $13
		dc.b   9
		dc.b $13
		dc.b   9
		dc.b $11
		dc.b   1
		dc.b $12
		dc.b   1
		dc.b $14
		dc.b   9
		dc.b $14
		dc.b   9
		dc.b $12
		dc.b $11
		dc.b $12
		dc.b $11
		dc.b $14
		dc.b $19
		dc.b $14
		dc.b $19
		dc.b $12
		dc.b $11
		dc.b $11
		dc.b $11
		dc.b $13
		dc.b $19
		dc.b $13
		dc.b $19
		dc.b $11
byte_130850:    dc.b 6
		dc.b   6
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $17
		dc.b   1
		dc.b $1A
		dc.b   9
		dc.b $1A
		dc.b   9
		dc.b $17
		dc.b   9
		dc.b   0
		dc.b   1
		dc.b $15
		dc.b   1
		dc.b $18
		dc.b   1
		dc.b $1B
		dc.b   9
		dc.b $1B
		dc.b   9
		dc.b $18
		dc.b   9
		dc.b $15
		dc.b   1
		dc.b $16
		dc.b   1
		dc.b $19
		dc.b   1
		dc.b $1C
		dc.b   9
		dc.b $1C
		dc.b   9
		dc.b $19
		dc.b   9
		dc.b $16
		dc.b $11
		dc.b $16
		dc.b $11
		dc.b $19
		dc.b $11
		dc.b $1C
		dc.b $19
		dc.b $1C
		dc.b $19
		dc.b $19
		dc.b $19
		dc.b $16
		dc.b $11
		dc.b $15
		dc.b $11
		dc.b $18
		dc.b $11
		dc.b $1B
		dc.b $19
		dc.b $1B
		dc.b $19
		dc.b $18
		dc.b $19
		dc.b $15
		dc.b $11
		dc.b   0
		dc.b $11
		dc.b $17
		dc.b $11
		dc.b $1A
		dc.b $19
		dc.b $1A
		dc.b $19
		dc.b $17
		dc.b $19
		dc.b   0
byte_13089A:    dc.b 8
		dc.b   8
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $22 
		dc.b   1
		dc.b $26 
		dc.b   9
		dc.b $26 
		dc.b   9
		dc.b $22 
		dc.b   9
		dc.b   0
		dc.b   9
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $1F
		dc.b   1
		dc.b $23 
		dc.b   1
		dc.b $27 
		dc.b   9
		dc.b $27 
		dc.b   9
		dc.b $23 
		dc.b   9
		dc.b $1F
		dc.b   9
		dc.b   0
		dc.b   1
		dc.b $1D
		dc.b   1
		dc.b $20
		dc.b   1
		dc.b $24 
		dc.b   1
		dc.b $28 
		dc.b   9
		dc.b $28 
		dc.b   9
		dc.b $24 
		dc.b   9
		dc.b $20
		dc.b   9
		dc.b $1D
		dc.b   1
		dc.b $1E
		dc.b   1
		dc.b $21 
		dc.b   1
		dc.b $25 
		dc.b   1
		dc.b $29 
		dc.b   9
		dc.b $29 
		dc.b   9
		dc.b $25 
		dc.b   9
		dc.b $21 
		dc.b   9
		dc.b $1E
		dc.b $11
		dc.b $1E
		dc.b $11
		dc.b $21 
		dc.b $11
		dc.b $25 
		dc.b $11
		dc.b $29 
		dc.b $19
		dc.b $29 
		dc.b $19
		dc.b $25 
		dc.b $19
		dc.b $21 
		dc.b $19
		dc.b $1E
		dc.b $11
		dc.b $1D
		dc.b $11
		dc.b $20
		dc.b $11
		dc.b $24 
		dc.b $11
		dc.b $28 
		dc.b $19
		dc.b $28 
		dc.b $19
		dc.b $24 
		dc.b $19
		dc.b $20
		dc.b $19
		dc.b $1D
		dc.b $11
		dc.b   0
		dc.b $11
		dc.b $1F
		dc.b $11
		dc.b $23 
		dc.b $11
		dc.b $27 
		dc.b $19
		dc.b $27 
		dc.b $19
		dc.b $23 
		dc.b $19
		dc.b $1F
		dc.b $19
		dc.b   0
		dc.b $11
		dc.b   0
		dc.b $11
		dc.b   0
		dc.b $11
		dc.b $22 
		dc.b $11
		dc.b $26 
		dc.b $19
		dc.b $26 
		dc.b $19
		dc.b $22 
		dc.b $19
		dc.b   0
		dc.b $19
		dc.b   0
byte_13091C:    dc.b $C
		dc.b  $C
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   9
		dc.b   0
		dc.b   9
		dc.b   0
		dc.b   9
		dc.b   0
		dc.b   9
		dc.b   0
		dc.b   9
		dc.b   0
		dc.b   9
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $2C 
		dc.b   1
		dc.b $2F 
		dc.b   1
		dc.b $38 
		dc.b   1
		dc.b $3B 
		dc.b   9
		dc.b $3B 
		dc.b   9
		dc.b $38 
		dc.b   9
		dc.b $2F 
		dc.b   9
		dc.b $2C 
		dc.b   9
		dc.b   0
		dc.b   9
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $2A 
		dc.b   1
		dc.b $2D 
		dc.b   1
		dc.b $30 
		dc.b   1
		dc.b $39 
		dc.b   1
		dc.b $3C 
		dc.b   9
		dc.b $3C 
		dc.b   9
		dc.b $39 
		dc.b   9
		dc.b $30 
		dc.b   9
		dc.b $2D 
		dc.b   9
		dc.b $2A 
		dc.b   9
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $2B 
		dc.b   1
		dc.b $2E 
		dc.b   1
		dc.b $31 
		dc.b   1
		dc.b $3A 
		dc.b   1
		dc.b $3D 
		dc.b   9
		dc.b $3D 
		dc.b   9
		dc.b $3A 
		dc.b   9
		dc.b $31 
		dc.b   9
		dc.b $2E 
		dc.b   9
		dc.b $2B 
		dc.b   9
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $32 
		dc.b   1
		dc.b $34 
		dc.b   1
		dc.b $36 
		dc.b   1
		dc.b $3E 
		dc.b   1
		dc.b $29 
		dc.b   9
		dc.b $29 
		dc.b   9
		dc.b $3E 
		dc.b   9
		dc.b $36 
		dc.b   9
		dc.b $34 
		dc.b   9
		dc.b $32 
		dc.b   9
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $33 
		dc.b   1
		dc.b $35 
		dc.b   1
		dc.b $37 
		dc.b   1
		dc.b $29 
		dc.b   1
		dc.b $29 
		dc.b   9
		dc.b $29 
		dc.b   9
		dc.b $29 
		dc.b   9
		dc.b $37 
		dc.b   9
		dc.b $35 
		dc.b   9
		dc.b $33 
		dc.b   9
		dc.b   0
		dc.b $11
		dc.b   0
		dc.b $11
		dc.b $33 
		dc.b $11
		dc.b $35 
		dc.b $11
		dc.b $37 
		dc.b $11
		dc.b $29 
		dc.b $11
		dc.b $29 
		dc.b $19
		dc.b $29 
		dc.b $19
		dc.b $29 
		dc.b $19
		dc.b $37 
		dc.b $19
		dc.b $35 
		dc.b $19
		dc.b $33 
		dc.b $19
		dc.b   0
		dc.b $11
		dc.b   0
		dc.b $11
		dc.b $32 
		dc.b $11
		dc.b $34 
		dc.b $11
		dc.b $36 
		dc.b $11
		dc.b $3E 
		dc.b $11
		dc.b $29 
		dc.b $19
		dc.b $29 
		dc.b $19
		dc.b $3E 
		dc.b $19
		dc.b $36 
		dc.b $19
		dc.b $34 
		dc.b $19
		dc.b $32 
		dc.b $19
		dc.b   0
		dc.b $11
		dc.b   0
		dc.b $11
		dc.b $2B 
		dc.b $11
		dc.b $2E 
		dc.b $11
		dc.b $31 
		dc.b $11
		dc.b $3A 
		dc.b $11
		dc.b $3D 
		dc.b $19
		dc.b $3D 
		dc.b $19
		dc.b $3A 
		dc.b $19
		dc.b $31 
		dc.b $19
		dc.b $2E 
		dc.b $19
		dc.b $2B 
		dc.b $19
		dc.b   0
		dc.b $11
		dc.b   0
		dc.b $11
		dc.b $2A 
		dc.b $11
		dc.b $2D 
		dc.b $11
		dc.b $30 
		dc.b $11
		dc.b $39 
		dc.b $11
		dc.b $3C 
		dc.b $19
		dc.b $3C 
		dc.b $19
		dc.b $39 
		dc.b $19
		dc.b $30 
		dc.b $19
		dc.b $2D 
		dc.b $19
		dc.b $2A 
		dc.b $19
		dc.b   0
		dc.b $11
		dc.b   0
		dc.b $11
		dc.b   0
		dc.b $11
		dc.b $2C 
		dc.b $11
		dc.b $2F 
		dc.b $11
		dc.b $38 
		dc.b $11
		dc.b $3B 
		dc.b $19
		dc.b $3B 
		dc.b $19
		dc.b $38 
		dc.b $19
		dc.b $2F 
		dc.b $19
		dc.b $2C 
		dc.b $19
		dc.b   0
		dc.b $19
		dc.b   0
		dc.b $11
		dc.b   0
		dc.b $11
		dc.b   0
		dc.b $11
		dc.b   0
		dc.b $11
		dc.b   0
		dc.b $11
		dc.b   0
		dc.b $11
		dc.b   0
		dc.b $19
		dc.b   0
		dc.b $19
		dc.b   0
		dc.b $19
		dc.b   0
		dc.b $19
		dc.b   0
		dc.b $19
		dc.b   0
		dc.b $19
		dc.b   0
byte_130A3E:    dc.b $10
		dc.b $10
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $22 
		dc.b   1
		dc.b $53 
		dc.b   1
		dc.b $56 
		dc.b   9
		dc.b $56 
		dc.b   9
		dc.b $53 
		dc.b   9
		dc.b $22 
		dc.b   9
		dc.b   0
		dc.b   9
		dc.b   0
		dc.b   9
		dc.b   0
		dc.b   9
		dc.b   0
		dc.b   9
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $40 
		dc.b   1
		dc.b $4F 
		dc.b   1
		dc.b $51 
		dc.b   1
		dc.b $54 
		dc.b   1
		dc.b $57 
		dc.b   9
		dc.b $57 
		dc.b   9
		dc.b $54 
		dc.b   9
		dc.b $51 
		dc.b   9
		dc.b $4F 
		dc.b   9
		dc.b $40 
		dc.b   9
		dc.b   0
		dc.b   9
		dc.b   0
		dc.b   9
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $3F 
		dc.b   1
		dc.b $41 
		dc.b   1
		dc.b $50 
		dc.b   1
		dc.b $52 
		dc.b   1
		dc.b $55 
		dc.b   1
		dc.b $58 
		dc.b   9
		dc.b $58 
		dc.b   9
		dc.b $55 
		dc.b   9
		dc.b $52 
		dc.b   9
		dc.b $50 
		dc.b   9
		dc.b $41 
		dc.b   9
		dc.b $3F 
		dc.b   9
		dc.b   0
		dc.b   9
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $43 
		dc.b   1
		dc.b $47 
		dc.b   1
		dc.b $4B 
		dc.b   1
		dc.b $59 
		dc.b   1
		dc.b $5D 
		dc.b   1
		dc.b $60 
		dc.b   1
		dc.b $62 
		dc.b   9
		dc.b $62 
		dc.b   9
		dc.b $60 
		dc.b   9
		dc.b $5D 
		dc.b   9
		dc.b $59 
		dc.b   9
		dc.b $4B 
		dc.b   9
		dc.b $47 
		dc.b   9
		dc.b $43 
		dc.b   9
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $44 
		dc.b   1
		dc.b $48 
		dc.b   1
		dc.b $4C 
		dc.b   1
		dc.b $5A 
		dc.b   1
		dc.b $5E 
		dc.b   1
		dc.b $61 
		dc.b   1
		dc.b $63 
		dc.b   9
		dc.b $63 
		dc.b   9
		dc.b $61 
		dc.b   9
		dc.b $5E 
		dc.b   9
		dc.b $5A 
		dc.b   9
		dc.b $4C 
		dc.b   9
		dc.b $48 
		dc.b   9
		dc.b $44 
		dc.b   9
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $45 
		dc.b   1
		dc.b $49 
		dc.b   1
		dc.b $4D 
		dc.b   1
		dc.b $5B 
		dc.b   1
		dc.b $5F 
		dc.b   1
		dc.b $29 
		dc.b   1
		dc.b $29 
		dc.b   9
		dc.b $29 
		dc.b   9
		dc.b $29 
		dc.b   9
		dc.b $5F 
		dc.b   9
		dc.b $5B 
		dc.b   9
		dc.b $4D 
		dc.b   9
		dc.b $49 
		dc.b   9
		dc.b $45 
		dc.b   9
		dc.b   0
		dc.b   1
		dc.b $42 
		dc.b   1
		dc.b $46 
		dc.b   1
		dc.b $4A 
		dc.b   1
		dc.b $4E 
		dc.b   1
		dc.b $5C 
		dc.b   1
		dc.b $29 
		dc.b   1
		dc.b $29 
		dc.b   1
		dc.b $29 
		dc.b   9
		dc.b $29 
		dc.b   9
		dc.b $29 
		dc.b   9
		dc.b $29 
		dc.b   9
		dc.b $5C 
		dc.b   9
		dc.b $4E 
		dc.b   9
		dc.b $4A 
		dc.b   9
		dc.b $46 
		dc.b   9
		dc.b $42 
		dc.b   1
		dc.b $64 
		dc.b   1
		dc.b $65 
		dc.b   1
		dc.b $66 
		dc.b   1
		dc.b $67 
		dc.b   1
		dc.b $68 
		dc.b   1
		dc.b $29 
		dc.b   1
		dc.b $29 
		dc.b   1
		dc.b $29 
		dc.b   9
		dc.b $29 
		dc.b   9
		dc.b $29 
		dc.b   9
		dc.b $29 
		dc.b   9
		dc.b $68 
		dc.b   9
		dc.b $67 
		dc.b   9
		dc.b $66 
		dc.b   9
		dc.b $65 
		dc.b   9
		dc.b $64 
		dc.b $11
		dc.b $64 
		dc.b $11
		dc.b $65 
		dc.b $11
		dc.b $66 
		dc.b $11
		dc.b $67 
		dc.b $11
		dc.b $68 
		dc.b $11
		dc.b $29 
		dc.b $11
		dc.b $29 
		dc.b $11
		dc.b $29 
		dc.b $19
		dc.b $29 
		dc.b $19
		dc.b $29 
		dc.b $19
		dc.b $29 
		dc.b $19
		dc.b $68 
		dc.b $19
		dc.b $67 
		dc.b $19
		dc.b $66 
		dc.b $19
		dc.b $65 
		dc.b $19
		dc.b $64 
		dc.b $11
		dc.b $42 
		dc.b $11
		dc.b $46 
		dc.b $11
		dc.b $4A 
		dc.b $11
		dc.b $4E 
		dc.b $11
		dc.b $5C 
		dc.b $11
		dc.b $29 
		dc.b $11
		dc.b $29 
		dc.b $11
		dc.b $29 
		dc.b $19
		dc.b $29 
		dc.b $19
		dc.b $29 
		dc.b $19
		dc.b $29 
		dc.b $19
		dc.b $5C 
		dc.b $19
		dc.b $4E 
		dc.b $19
		dc.b $4A 
		dc.b $19
		dc.b $46 
		dc.b $19
		dc.b $42 
		dc.b $11
		dc.b   0
		dc.b $11
		dc.b $45 
		dc.b $11
		dc.b $49 
		dc.b $11
		dc.b $4D 
		dc.b $11
		dc.b $5B 
		dc.b $11
		dc.b $5F 
		dc.b $11
		dc.b $29 
		dc.b $11
		dc.b $29 
		dc.b $19
		dc.b $29 
		dc.b $19
		dc.b $29 
		dc.b $19
		dc.b $5F 
		dc.b $19
		dc.b $5B 
		dc.b $19
		dc.b $4D 
		dc.b $19
		dc.b $49 
		dc.b $19
		dc.b $45 
		dc.b $19
		dc.b   0
		dc.b $11
		dc.b   0
		dc.b $11
		dc.b $44 
		dc.b $11
		dc.b $48 
		dc.b $11
		dc.b $4C 
		dc.b $11
		dc.b $5A 
		dc.b $11
		dc.b $5E 
		dc.b $11
		dc.b $61 
		dc.b $11
		dc.b $63 
		dc.b $19
		dc.b $63 
		dc.b $19
		dc.b $61 
		dc.b $19
		dc.b $5E 
		dc.b $19
		dc.b $5A 
		dc.b $19
		dc.b $4C 
		dc.b $19
		dc.b $48 
		dc.b $19
		dc.b $44 
		dc.b $19
		dc.b   0
		dc.b $11
		dc.b   0
		dc.b $11
		dc.b $43 
		dc.b $11
		dc.b $47 
		dc.b $11
		dc.b $4B 
		dc.b $11
		dc.b $59 
		dc.b $11
		dc.b $5D 
		dc.b $11
		dc.b $60 
		dc.b $11
		dc.b $62 
		dc.b $19
		dc.b $62 
		dc.b $19
		dc.b $60 
		dc.b $19
		dc.b $5D 
		dc.b $19
		dc.b $59 
		dc.b $19
		dc.b $4B 
		dc.b $19
		dc.b $47 
		dc.b $19
		dc.b $43 
		dc.b $19
		dc.b   0
		dc.b $11
		dc.b   0
		dc.b $11
		dc.b   0
		dc.b $11
		dc.b $3F 
		dc.b $11
		dc.b $41 
		dc.b $11
		dc.b $50 
		dc.b $11
		dc.b $52 
		dc.b $11
		dc.b $55 
		dc.b $11
		dc.b $58 
		dc.b $19
		dc.b $58 
		dc.b $19
		dc.b $55 
		dc.b $19
		dc.b $52 
		dc.b $19
		dc.b $50 
		dc.b $19
		dc.b $41 
		dc.b $19
		dc.b $3F 
		dc.b $19
		dc.b   0
		dc.b $19
		dc.b   0
		dc.b $11
		dc.b   0
		dc.b $11
		dc.b   0
		dc.b $11
		dc.b   0
		dc.b $11
		dc.b $40 
		dc.b $11
		dc.b $4F 
		dc.b $11
		dc.b $51 
		dc.b $11
		dc.b $54 
		dc.b $11
		dc.b $57 
		dc.b $19
		dc.b $57 
		dc.b $19
		dc.b $54 
		dc.b $19
		dc.b $51 
		dc.b $19
		dc.b $4F 
		dc.b $19
		dc.b $40 
		dc.b $19
		dc.b   0
		dc.b $19
		dc.b   0
		dc.b $19
		dc.b   0
		dc.b $11
		dc.b   0
		dc.b $11
		dc.b   0
		dc.b $11
		dc.b   0
		dc.b $11
		dc.b   0
		dc.b $11
		dc.b   0
		dc.b $11
		dc.b $22 
		dc.b $11
		dc.b $53 
		dc.b $11
		dc.b $56 
		dc.b $19
		dc.b $56 
		dc.b $19
		dc.b $53 
		dc.b $19
		dc.b $22 
		dc.b $19
		dc.b   0
		dc.b $19
		dc.b   0
		dc.b $19
		dc.b   0
		dc.b $19
		dc.b   0
		dc.b $19
		dc.b   0
byte_130C40:    dc.b $16
		dc.b $16
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $82 
		dc.b   1
		dc.b $86 
		dc.b   1
		dc.b $8A 
		dc.b   1
		dc.b $A4 
		dc.b   1
		dc.b $A8 
		dc.b   1
		dc.b $A8 
		dc.b   9
		dc.b $A8 
		dc.b   9
		dc.b $A8 
		dc.b   9
		dc.b $A4 
		dc.b   9
		dc.b $8A 
		dc.b   9
		dc.b $86 
		dc.b   9
		dc.b $82 
		dc.b   9
		dc.b   0
		dc.b   9
		dc.b   0
		dc.b   9
		dc.b   0
		dc.b   9
		dc.b   0
		dc.b   9
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $80 
		dc.b   1
		dc.b $83 
		dc.b   1
		dc.b $87 
		dc.b   1
		dc.b $8B 
		dc.b   1
		dc.b $A5 
		dc.b   1
		dc.b $57 
		dc.b   1
		dc.b $57 
		dc.b   9
		dc.b $57 
		dc.b   9
		dc.b $57 
		dc.b   9
		dc.b $A5 
		dc.b   9
		dc.b $8B 
		dc.b   9
		dc.b $87 
		dc.b   9
		dc.b $83 
		dc.b   9
		dc.b $80 
		dc.b   9
		dc.b   0
		dc.b   9
		dc.b   0
		dc.b   9
		dc.b   0
		dc.b   9
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $6A 
		dc.b   1
		dc.b $6B 
		dc.b   1
		dc.b $84 
		dc.b   1
		dc.b $88 
		dc.b   1
		dc.b $8C 
		dc.b   1
		dc.b $A6 
		dc.b   1
		dc.b $A9 
		dc.b   1
		dc.b $AB 
		dc.b   9
		dc.b $AB 
		dc.b   9
		dc.b $A9 
		dc.b   9
		dc.b $A6 
		dc.b   9
		dc.b $8C 
		dc.b   9
		dc.b $88 
		dc.b   9
		dc.b $84 
		dc.b   9
		dc.b $6B 
		dc.b   9
		dc.b $6A 
		dc.b   9
		dc.b   0
		dc.b   9
		dc.b   0
		dc.b   9
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $69 
		dc.b   1
		dc.b $6B 
		dc.b   1
		dc.b $81 
		dc.b   1
		dc.b $85 
		dc.b   1
		dc.b $89 
		dc.b   1
		dc.b $8D 
		dc.b   1
		dc.b $A7 
		dc.b   1
		dc.b $AA 
		dc.b   1
		dc.b $AC 
		dc.b   9
		dc.b $AC 
		dc.b   9
		dc.b $AA 
		dc.b   9
		dc.b $A7 
		dc.b   9
		dc.b $8D 
		dc.b   9
		dc.b $89 
		dc.b   9
		dc.b $85 
		dc.b   9
		dc.b $81 
		dc.b   9
		dc.b $6B 
		dc.b   9
		dc.b $69 
		dc.b   9
		dc.b   0
		dc.b   9
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $2A 
		dc.b   1
		dc.b $6F 
		dc.b   1
		dc.b $73 
		dc.b   1
		dc.b $8E 
		dc.b   1
		dc.b $92 
		dc.b   1
		dc.b $96 
		dc.b   1
		dc.b $9A 
		dc.b   1
		dc.b $AD 
		dc.b   1
		dc.b $AF 
		dc.b   1
		dc.b $B1 
		dc.b   9
		dc.b $B1 
		dc.b   9
		dc.b $AF 
		dc.b   9
		dc.b $AD 
		dc.b   9
		dc.b $9A 
		dc.b   9
		dc.b $96 
		dc.b   9
		dc.b $92 
		dc.b   9
		dc.b $8E 
		dc.b   9
		dc.b $73 
		dc.b   9
		dc.b $6F 
		dc.b   9
		dc.b $2A 
		dc.b   9
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $6C 
		dc.b   1
		dc.b $70 
		dc.b   1
		dc.b $74 
		dc.b   1
		dc.b $8F 
		dc.b   1
		dc.b $93 
		dc.b   1
		dc.b $97 
		dc.b   1
		dc.b $9B 
		dc.b   1
		dc.b $AE 
		dc.b   1
		dc.b $B0 
		dc.b   1
		dc.b $B2 
		dc.b   9
		dc.b $B2 
		dc.b   9
		dc.b $B0 
		dc.b   9
		dc.b $AE 
		dc.b   9
		dc.b $9B 
		dc.b   9
		dc.b $97 
		dc.b   9
		dc.b $93 
		dc.b   9
		dc.b $8F 
		dc.b   9
		dc.b $74 
		dc.b   9
		dc.b $70 
		dc.b   9
		dc.b $6C 
		dc.b   9
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $6D 
		dc.b   1
		dc.b $71 
		dc.b   1
		dc.b $75 
		dc.b   1
		dc.b $90 
		dc.b   1
		dc.b $94 
		dc.b   1
		dc.b $98 
		dc.b   1
		dc.b $9C 
		dc.b   1
		dc.b $29 
		dc.b   1
		dc.b $29 
		dc.b   1
		dc.b $29 
		dc.b   9
		dc.b $29 
		dc.b   9
		dc.b $29 
		dc.b   9
		dc.b $29 
		dc.b   9
		dc.b $9C 
		dc.b   9
		dc.b $98 
		dc.b   9
		dc.b $94 
		dc.b   9
		dc.b $90 
		dc.b   9
		dc.b $75 
		dc.b   9
		dc.b $71 
		dc.b   9
		dc.b $6D 
		dc.b   9
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $6E 
		dc.b   1
		dc.b $72 
		dc.b   1
		dc.b $76 
		dc.b   1
		dc.b $91 
		dc.b   1
		dc.b $95 
		dc.b   1
		dc.b $99 
		dc.b   1
		dc.b $29 
		dc.b   1
		dc.b $29 
		dc.b   1
		dc.b $29 
		dc.b   1
		dc.b $29 
		dc.b   9
		dc.b $29 
		dc.b   9
		dc.b $29 
		dc.b   9
		dc.b $29 
		dc.b   9
		dc.b $29 
		dc.b   9
		dc.b $99 
		dc.b   9
		dc.b $95 
		dc.b   9
		dc.b $91 
		dc.b   9
		dc.b $76 
		dc.b   9
		dc.b $72 
		dc.b   9
		dc.b $6E 
		dc.b   9
		dc.b   0
		dc.b   1
		dc.b $77 
		dc.b   1
		dc.b $78 
		dc.b   1
		dc.b $7A 
		dc.b   1
		dc.b $7D 
		dc.b   1
		dc.b $9D 
		dc.b   1
		dc.b $A0 
		dc.b   1
		dc.b $A3 
		dc.b   1
		dc.b $29 
		dc.b   1
		dc.b $29 
		dc.b   1
		dc.b $29 
		dc.b   1
		dc.b $29 
		dc.b   9
		dc.b $29 
		dc.b   9
		dc.b $29 
		dc.b   9
		dc.b $29 
		dc.b   9
		dc.b $29 
		dc.b   9
		dc.b $A3 
		dc.b   9
		dc.b $A0 
		dc.b   9
		dc.b $9D 
		dc.b   9
		dc.b $7D 
		dc.b   9
		dc.b $7A 
		dc.b   9
		dc.b $78 
		dc.b   9
		dc.b $77 
		dc.b   1
		dc.b $64 
		dc.b   1
		dc.b $79 
		dc.b   1
		dc.b $7B 
		dc.b   1
		dc.b $7E 
		dc.b   1
		dc.b $9E 
		dc.b   1
		dc.b $A1 
		dc.b   1
		dc.b $29 
		dc.b   1
		dc.b $29 
		dc.b   1
		dc.b $29 
		dc.b   1
		dc.b $29 
		dc.b   1
		dc.b $29 
		dc.b   9
		dc.b $29 
		dc.b   9
		dc.b $29 
		dc.b   9
		dc.b $29 
		dc.b   9
		dc.b $29 
		dc.b   9
		dc.b $29 
		dc.b   9
		dc.b $A1 
		dc.b   9
		dc.b $9E 
		dc.b   9
		dc.b $7E 
		dc.b   9
		dc.b $7B 
		dc.b   9
		dc.b $79 
		dc.b   9
		dc.b $64 
		dc.b   1
		dc.b $64 
		dc.b   1
		dc.b $79 
		dc.b   1
		dc.b $7C 
		dc.b   1
		dc.b $7F 
		dc.b   1
		dc.b $9F 
		dc.b   1
		dc.b $A2 
		dc.b   1
		dc.b $29 
		dc.b   1
		dc.b $29 
		dc.b   1
		dc.b $29 
		dc.b   1
		dc.b $29 
		dc.b   1
		dc.b $29 
		dc.b   9
		dc.b $29 
		dc.b   9
		dc.b $29 
		dc.b   9
		dc.b $29 
		dc.b   9
		dc.b $29 
		dc.b   9
		dc.b $29 
		dc.b   9
		dc.b $A2 
		dc.b   9
		dc.b $9F 
		dc.b   9
		dc.b $7F 
		dc.b   9
		dc.b $7C 
		dc.b   9
		dc.b $79 
		dc.b   9
		dc.b $64 
		dc.b $11
		dc.b $64 
		dc.b $11
		dc.b $79 
		dc.b $11
		dc.b $7C 
		dc.b $11
		dc.b $7F 
		dc.b $11
		dc.b $9F 
		dc.b $11
		dc.b $A2 
		dc.b $11
		dc.b $29 
		dc.b $11
		dc.b $29 
		dc.b $11
		dc.b $29 
		dc.b $11
		dc.b $29 
		dc.b $11
		dc.b $29 
		dc.b $19
		dc.b $29 
		dc.b $19
		dc.b $29 
		dc.b $19
		dc.b $29 
		dc.b $19
		dc.b $29 
		dc.b $19
		dc.b $29 
		dc.b $19
		dc.b $A2 
		dc.b $19
		dc.b $9F 
		dc.b $19
		dc.b $7F 
		dc.b $19
		dc.b $7C 
		dc.b $19
		dc.b $79 
		dc.b $19
		dc.b $64 
		dc.b $11
		dc.b $64 
		dc.b $11
		dc.b $79 
		dc.b $11
		dc.b $7B 
		dc.b $11
		dc.b $7E 
		dc.b $11
		dc.b $9E 
		dc.b $11
		dc.b $A1 
		dc.b $11
		dc.b $29 
		dc.b $11
		dc.b $29 
		dc.b $11
		dc.b $29 
		dc.b $11
		dc.b $29 
		dc.b $11
		dc.b $29 
		dc.b $19
		dc.b $29 
		dc.b $19
		dc.b $29 
		dc.b $19
		dc.b $29 
		dc.b $19
		dc.b $29 
		dc.b $19
		dc.b $29 
		dc.b $19
		dc.b $A1 
		dc.b $19
		dc.b $9E 
		dc.b $19
		dc.b $7E 
		dc.b $19
		dc.b $7B 
		dc.b $19
		dc.b $79 
		dc.b $19
		dc.b $64 
		dc.b $11
		dc.b $77 
		dc.b $11
		dc.b $78 
		dc.b $11
		dc.b $7A 
		dc.b $11
		dc.b $7D 
		dc.b $11
		dc.b $9D 
		dc.b $11
		dc.b $A0 
		dc.b $11
		dc.b $A3 
		dc.b $11
		dc.b $29 
		dc.b $11
		dc.b $29 
		dc.b $11
		dc.b $29 
		dc.b $11
		dc.b $29 
		dc.b $19
		dc.b $29 
		dc.b $19
		dc.b $29 
		dc.b $19
		dc.b $29 
		dc.b $19
		dc.b $29 
		dc.b $19
		dc.b $A3 
		dc.b $19
		dc.b $A0 
		dc.b $19
		dc.b $9D 
		dc.b $19
		dc.b $7D 
		dc.b $19
		dc.b $7A 
		dc.b $19
		dc.b $78 
		dc.b $19
		dc.b $77 
		dc.b $11
		dc.b   0
		dc.b $11
		dc.b $6E 
		dc.b $11
		dc.b $72 
		dc.b $11
		dc.b $76 
		dc.b $11
		dc.b $91 
		dc.b $11
		dc.b $95 
		dc.b $11
		dc.b $99 
		dc.b $11
		dc.b $29 
		dc.b $11
		dc.b $29 
		dc.b $11
		dc.b $29 
		dc.b $11
		dc.b $29 
		dc.b $19
		dc.b $29 
		dc.b $19
		dc.b $29 
		dc.b $19
		dc.b $29 
		dc.b $19
		dc.b $29 
		dc.b $19
		dc.b $99 
		dc.b $19
		dc.b $95 
		dc.b $19
		dc.b $91 
		dc.b $19
		dc.b $76 
		dc.b $19
		dc.b $72 
		dc.b $19
		dc.b $6E 
		dc.b $19
		dc.b   0
		dc.b $11
		dc.b   0
		dc.b $11
		dc.b $6D 
		dc.b $11
		dc.b $71 
		dc.b $11
		dc.b $75 
		dc.b $11
		dc.b $90 
		dc.b $11
		dc.b $94 
		dc.b $11
		dc.b $98 
		dc.b $11
		dc.b $9C 
		dc.b $11
		dc.b $29 
		dc.b $11
		dc.b $29 
		dc.b $11
		dc.b $29 
		dc.b $19
		dc.b $29 
		dc.b $19
		dc.b $29 
		dc.b $19
		dc.b $29 
		dc.b $19
		dc.b $9C 
		dc.b $19
		dc.b $98 
		dc.b $19
		dc.b $94 
		dc.b $19
		dc.b $90 
		dc.b $19
		dc.b $75 
		dc.b $19
		dc.b $71 
		dc.b $19
		dc.b $6D 
		dc.b $19
		dc.b   0
		dc.b $11
		dc.b   0
		dc.b $11
		dc.b $6C 
		dc.b $11
		dc.b $70 
		dc.b $11
		dc.b $74 
		dc.b $11
		dc.b $8F 
		dc.b $11
		dc.b $93 
		dc.b $11
		dc.b $97 
		dc.b $11
		dc.b $9B 
		dc.b $11
		dc.b $AE 
		dc.b $11
		dc.b $B0 
		dc.b $11
		dc.b $B2 
		dc.b $19
		dc.b $B2 
		dc.b $19
		dc.b $B0 
		dc.b $19
		dc.b $AE 
		dc.b $19
		dc.b $9B 
		dc.b $19
		dc.b $97 
		dc.b $19
		dc.b $93 
		dc.b $19
		dc.b $8F 
		dc.b $19
		dc.b $74 
		dc.b $19
		dc.b $70 
		dc.b $19
		dc.b $6C 
		dc.b $19
		dc.b   0
		dc.b $11
		dc.b   0
		dc.b $11
		dc.b $2A 
		dc.b $11
		dc.b $6F 
		dc.b $11
		dc.b $73 
		dc.b $11
		dc.b $8E 
		dc.b $11
		dc.b $92 
		dc.b $11
		dc.b $96 
		dc.b $11
		dc.b $9A 
		dc.b $11
		dc.b $AD 
		dc.b $11
		dc.b $AF 
		dc.b $11
		dc.b $B1 
		dc.b $19
		dc.b $B1 
		dc.b $19
		dc.b $AF 
		dc.b $19
		dc.b $AD 
		dc.b $19
		dc.b $9A 
		dc.b $19
		dc.b $96 
		dc.b $19
		dc.b $92 
		dc.b $19
		dc.b $8E 
		dc.b $19
		dc.b $73 
		dc.b $19
		dc.b $6F 
		dc.b $19
		dc.b $2A 
		dc.b $19
		dc.b   0
		dc.b $11
		dc.b   0
		dc.b $11
		dc.b   0
		dc.b $11
		dc.b $69 
		dc.b $11
		dc.b $6B 
		dc.b $11
		dc.b $81 
		dc.b $11
		dc.b $85 
		dc.b $11
		dc.b $89 
		dc.b $11
		dc.b $8D 
		dc.b $11
		dc.b $A7 
		dc.b $11
		dc.b $AA 
		dc.b $11
		dc.b $AC 
		dc.b $19
		dc.b $AC 
		dc.b $19
		dc.b $AA 
		dc.b $19
		dc.b $A7 
		dc.b $19
		dc.b $8D 
		dc.b $19
		dc.b $89 
		dc.b $19
		dc.b $85 
		dc.b $19
		dc.b $81 
		dc.b $19
		dc.b $6B 
		dc.b $19
		dc.b $69 
		dc.b $19
		dc.b   0
		dc.b $19
		dc.b   0
		dc.b $11
		dc.b   0
		dc.b $11
		dc.b   0
		dc.b $11
		dc.b   0
		dc.b $11
		dc.b $6A 
		dc.b $11
		dc.b $6B 
		dc.b $11
		dc.b $84 
		dc.b $11
		dc.b $88 
		dc.b $11
		dc.b $8C 
		dc.b $11
		dc.b $A6 
		dc.b $11
		dc.b $A9 
		dc.b $11
		dc.b $AB 
		dc.b $19
		dc.b $AB 
		dc.b $19
		dc.b $A9 
		dc.b $19
		dc.b $A6 
		dc.b $19
		dc.b $8C 
		dc.b $19
		dc.b $88 
		dc.b $19
		dc.b $84 
		dc.b $19
		dc.b $6B 
		dc.b $19
		dc.b $6A 
		dc.b $19
		dc.b   0
		dc.b $19
		dc.b   0
		dc.b $19
		dc.b   0
		dc.b $11
		dc.b   0
		dc.b $11
		dc.b   0
		dc.b $11
		dc.b   0
		dc.b $11
		dc.b   0
		dc.b $11
		dc.b $80 
		dc.b $11
		dc.b $83 
		dc.b $11
		dc.b $87 
		dc.b $11
		dc.b $8B 
		dc.b $11
		dc.b $A5 
		dc.b $11
		dc.b $57 
		dc.b $11
		dc.b $57 
		dc.b $19
		dc.b $57 
		dc.b $19
		dc.b $57 
		dc.b $19
		dc.b $A5 
		dc.b $19
		dc.b $8B 
		dc.b $19
		dc.b $87 
		dc.b $19
		dc.b $83 
		dc.b $19
		dc.b $80 
		dc.b $19
		dc.b   0
		dc.b $19
		dc.b   0
		dc.b $19
		dc.b   0
		dc.b $19
		dc.b   0
		dc.b $11
		dc.b   0
		dc.b $11
		dc.b   0
		dc.b $11
		dc.b   0
		dc.b $11
		dc.b   0
		dc.b $11
		dc.b   0
		dc.b $11
		dc.b $82 
		dc.b $11
		dc.b $86 
		dc.b $11
		dc.b $8A 
		dc.b $11
		dc.b $A4 
		dc.b $11
		dc.b $A8 
		dc.b $11
		dc.b $A8 
		dc.b $19
		dc.b $A8 
		dc.b $19
		dc.b $A8 
		dc.b $19
		dc.b $A4 
		dc.b $19
		dc.b $8A 
		dc.b $19
		dc.b $86 
		dc.b $19
		dc.b $82 
		dc.b $19
		dc.b   0
		dc.b $19
		dc.b   0
		dc.b $19
		dc.b   0
		dc.b $19
		dc.b   0
		dc.b $19
		dc.b   0
byte_13100A:    dc.b $A
		dc.b  $D
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   1
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   2
		dc.b   1
		dc.b   3
		dc.b   1
		dc.b   4
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   5
		dc.b   1
		dc.b   6
		dc.b   1
		dc.b   7
		dc.b   1
		dc.b   8
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   9
		dc.b   1
		dc.b  $A
		dc.b   1
		dc.b  $B
		dc.b   1
		dc.b  $C
		dc.b   1
		dc.b  $D
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b  $E
		dc.b   1
		dc.b  $F
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $10
		dc.b   1
		dc.b $11
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $12
		dc.b   1
		dc.b $13
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $14
		dc.b   1
		dc.b $15
		dc.b   1
		dc.b $16
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $17
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $18
		dc.b   1
		dc.b $19
		dc.b   1
		dc.b $1A
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $1B
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $1C
		dc.b   1
		dc.b $1D
		dc.b   1
		dc.b $1E
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $1F
		dc.b   1
		dc.b $20
		dc.b   1
		dc.b $21 
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $22 
		dc.b   1
		dc.b $23 
		dc.b   1
		dc.b $24 
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $25 
		dc.b   1
		dc.b $26 
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $27 
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $28 
		dc.b   1
		dc.b $29 
		dc.b   1
		dc.b $2A 
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $2B 
		dc.b   1
		dc.b $2C 
		dc.b   1
		dc.b $2D 
		dc.b   1
		dc.b $2E 
		dc.b   1
		dc.b $2F 
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $30 
		dc.b   1
		dc.b $31 
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $32 
		dc.b   1
		dc.b $33 
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $34 
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $35 
		dc.b   1
		dc.b $36 
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $37 
		dc.b   1
		dc.b   0
byte_131110:    dc.b $A
		dc.b  $D
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   1
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   2
		dc.b   1
		dc.b   3
		dc.b   1
		dc.b   4
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   5
		dc.b   1
		dc.b   6
		dc.b   1
		dc.b   7
		dc.b   1
		dc.b   8
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   9
		dc.b   1
		dc.b  $A
		dc.b   1
		dc.b  $B
		dc.b   1
		dc.b  $C
		dc.b   1
		dc.b  $D
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b  $E
		dc.b   1
		dc.b  $F
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $10
		dc.b   1
		dc.b $11
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $12
		dc.b   1
		dc.b $13
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $14
		dc.b   1
		dc.b $15
		dc.b   1
		dc.b $16
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $17
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $18
		dc.b   1
		dc.b $19
		dc.b   1
		dc.b $1A
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $1B
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $1C
		dc.b   1
		dc.b $38 
		dc.b   1
		dc.b $1E
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $1F
		dc.b   1
		dc.b $39 
		dc.b   1
		dc.b $21 
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $3A 
		dc.b   1
		dc.b $3B 
		dc.b   1
		dc.b $24 
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $25 
		dc.b   1
		dc.b $26 
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $27 
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $3C 
		dc.b   1
		dc.b $3D 
		dc.b   1
		dc.b $2A 
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $2B 
		dc.b   1
		dc.b $2C 
		dc.b   1
		dc.b $2D 
		dc.b   1
		dc.b $3E 
		dc.b   1
		dc.b $3F 
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $30 
		dc.b   1
		dc.b $31 
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $32 
		dc.b   1
		dc.b $33 
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $40 
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $35 
		dc.b   1
		dc.b $36 
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $37 
		dc.b   1
		dc.b   0
byte_131216:    dc.b $A
		dc.b  $D
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   1
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   2
		dc.b   1
		dc.b   3
		dc.b   1
		dc.b   4
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   5
		dc.b   1
		dc.b   6
		dc.b   1
		dc.b   7
		dc.b   1
		dc.b   8
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   9
		dc.b   1
		dc.b  $A
		dc.b   1
		dc.b  $B
		dc.b   1
		dc.b  $C
		dc.b   1
		dc.b  $D
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b  $E
		dc.b   1
		dc.b  $F
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $10
		dc.b   1
		dc.b $11
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $12
		dc.b   1
		dc.b $13
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $14
		dc.b   1
		dc.b $15
		dc.b   1
		dc.b $16
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $17
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $18
		dc.b   1
		dc.b $19
		dc.b   1
		dc.b $1A
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $1B
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $1C
		dc.b   1
		dc.b $38 
		dc.b   1
		dc.b $1E
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $1F
		dc.b   1
		dc.b $39 
		dc.b   1
		dc.b $21 
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $3A 
		dc.b   1
		dc.b $41 
		dc.b   1
		dc.b $24 
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $25 
		dc.b   1
		dc.b $26 
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $27 
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $3C 
		dc.b   1
		dc.b $42 
		dc.b   1
		dc.b $2A 
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $2B 
		dc.b   1
		dc.b $2C 
		dc.b   1
		dc.b $2D 
		dc.b   1
		dc.b $3E 
		dc.b   1
		dc.b $3F 
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $43 
		dc.b   1
		dc.b $44 
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $32 
		dc.b   1
		dc.b $33 
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $40 
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $35 
		dc.b   1
		dc.b $36 
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $37 
		dc.b   1
		dc.b   0
byte_13131C:    dc.b $A
		dc.b  $D
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   1
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   2
		dc.b   1
		dc.b   3
		dc.b   1
		dc.b   4
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   5
		dc.b   1
		dc.b   6
		dc.b   1
		dc.b   7
		dc.b   1
		dc.b   8
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   9
		dc.b   1
		dc.b  $A
		dc.b   1
		dc.b  $B
		dc.b   1
		dc.b  $C
		dc.b   1
		dc.b  $D
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b  $E
		dc.b   1
		dc.b  $F
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $10
		dc.b   1
		dc.b $11
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $12
		dc.b   1
		dc.b $13
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $14
		dc.b   1
		dc.b $15
		dc.b   1
		dc.b $16
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $17
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $18
		dc.b   1
		dc.b $19
		dc.b   1
		dc.b $1A
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $1B
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $1C
		dc.b   1
		dc.b $1D
		dc.b   1
		dc.b $1E
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $1F
		dc.b   1
		dc.b $20
		dc.b   1
		dc.b $21 
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $45 
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $22 
		dc.b   1
		dc.b $23 
		dc.b   1
		dc.b $24 
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $46 
		dc.b   1
		dc.b $47 
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $48 
		dc.b   1
		dc.b $49 
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $28 
		dc.b   1
		dc.b $29 
		dc.b   1
		dc.b $2A 
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $4A 
		dc.b   1
		dc.b $4B 
		dc.b   1
		dc.b $4C 
		dc.b   1
		dc.b $4D 
		dc.b   1
		dc.b $4E 
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $30 
		dc.b   1
		dc.b $31 
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $4F 
		dc.b   1
		dc.b $50 
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $51 
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $52 
		dc.b   1
		dc.b $53 
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $54 
		dc.b   1
		dc.b   0
byte_131422:    dc.b $A
		dc.b  $D
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   1
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   2
		dc.b   1
		dc.b   3
		dc.b   1
		dc.b   4
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   5
		dc.b   1
		dc.b   6
		dc.b   1
		dc.b   7
		dc.b   1
		dc.b   8
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   9
		dc.b   1
		dc.b  $A
		dc.b   1
		dc.b  $B
		dc.b   1
		dc.b  $C
		dc.b   1
		dc.b  $D
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b  $E
		dc.b   1
		dc.b  $F
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $10
		dc.b   1
		dc.b $11
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $12
		dc.b   1
		dc.b $13
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $14
		dc.b   1
		dc.b $15
		dc.b   1
		dc.b $16
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $17
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $18
		dc.b   1
		dc.b $19
		dc.b   1
		dc.b $1A
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $1B
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $1C
		dc.b   1
		dc.b $38 
		dc.b   1
		dc.b $1E
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $1F
		dc.b   1
		dc.b $39 
		dc.b   1
		dc.b $21 
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $45 
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $3A 
		dc.b   1
		dc.b $3B 
		dc.b   1
		dc.b $24 
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $46 
		dc.b   1
		dc.b $47 
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $48 
		dc.b   1
		dc.b $49 
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $3C 
		dc.b   1
		dc.b $3D 
		dc.b   1
		dc.b $2A 
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $4A 
		dc.b   1
		dc.b $4B 
		dc.b   1
		dc.b $4C 
		dc.b   1
		dc.b $4D 
		dc.b   1
		dc.b $4E 
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $30 
		dc.b   1
		dc.b $31 
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $4F 
		dc.b   1
		dc.b $50 
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $51 
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $52 
		dc.b   1
		dc.b $53 
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $54 
		dc.b   1
		dc.b   0
byte_131528:    dc.b $A
		dc.b  $D
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   1
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   2
		dc.b   1
		dc.b   3
		dc.b   1
		dc.b   4
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   5
		dc.b   1
		dc.b   6
		dc.b   1
		dc.b   7
		dc.b   1
		dc.b   8
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   9
		dc.b   1
		dc.b  $A
		dc.b   1
		dc.b  $B
		dc.b   1
		dc.b  $C
		dc.b   1
		dc.b  $D
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b  $E
		dc.b   1
		dc.b  $F
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $10
		dc.b   1
		dc.b $11
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $12
		dc.b   1
		dc.b $13
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $14
		dc.b   1
		dc.b $15
		dc.b   1
		dc.b $16
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $17
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $18
		dc.b   1
		dc.b $19
		dc.b   1
		dc.b $1A
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $1B
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $1C
		dc.b   1
		dc.b $38 
		dc.b   1
		dc.b $1E
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $1F
		dc.b   1
		dc.b $39 
		dc.b   1
		dc.b $21 
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $45 
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $3A 
		dc.b   1
		dc.b $41 
		dc.b   1
		dc.b $24 
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $46 
		dc.b   1
		dc.b $47 
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $48 
		dc.b   1
		dc.b $49 
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $3C 
		dc.b   1
		dc.b $42 
		dc.b   1
		dc.b $2A 
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $4A 
		dc.b   1
		dc.b $4B 
		dc.b   1
		dc.b $4C 
		dc.b   1
		dc.b $4D 
		dc.b   1
		dc.b $4E 
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $43 
		dc.b   1
		dc.b $44 
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $4F 
		dc.b   1
		dc.b $50 
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $51 
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $52 
		dc.b   1
		dc.b $53 
		dc.b   1
		dc.b   0
		dc.b   1
		dc.b $54 
		dc.b   1
		dc.b   0

; =============== S U B R O U T I N E =======================================

InitDisplay_Intro:
		
		jsr     (j_DisableDisplayAndInterrupts).l
		moveq   #$3F,d0 
		jsr     (j_InitSprites).l
		jsr     (sub_324).l
		move.w  #$8C00,d0       ; H32 cell mode, no interlace
		jsr     (j_SetVdpReg).w
		move.w  #$9010,d0       ; scroll size : V64 cell, H32 cell
		jsr     (j_SetVdpReg).w
		move.w  #$8238,d0       ; scroll A table VRAM address : E000
		jsr     (j_SetVdpReg).w
		move.w  #$8407,d0       ; scroll B table VRAM address : E000
		jsr     (j_SetVdpReg).w
		move.w  #$8B03,d0       ; disable external interrupt, scanline H scroll
		jsr     (j_SetVdpReg).w
		move.w  #0,d6
		jsr     (j_UpdateForegroundVScrollData).w
		move.w  #$100,d6
		jsr     (j_UpdateBackgroundVScrollData).w
		jsr     (j_EnableDmaQueueProcessing).w
		lea     (PLANE_A_MAP_LAYOUT).l,a0
		move.w  #$3FF,d7
loc_131688:
		clr.l   (a0)+
		dbf     d7,loc_131688
                
		lea     (PLANE_A_MAP_LAYOUT).l,a0
		lea     ($E000).l,a1
		move.w  #$800,d0
		move.w  #2,d1
		jmp     (j_ApplyImmediateVramDma).l

    ; End of function InitDisplay_Intro


; =============== S U B R O U T I N E =======================================

sub_1316A8:
		movea.l (off_10034).l,a0
		lea     (byte_FFD140).l,a1
		move.w  #$300,d7
		jsr     (j_CopyBytes).l
		lea     (PLANE_A_MAP_LAYOUT).l,a0
		lea     ($E000).l,a1
		move.w  #$800,d0
		move.w  #2,d1
		jmp     (j_ApplyImmediateVramDma).l

    ; End of function sub_1316A8


; =============== S U B R O U T I N E =======================================

sub_1316D8:
		movea.l (off_10034).l,a0
		lea     (byte_FFD140).l,a1
		move.w  #$300,d7
		jsr     (j_CopyBytes).l
		lea     byte_13171C(pc), a0
		lea     (byte_FFD940).l,a1
		move.w  #$300,d7
		jsr     (j_CopyBytes).l
		lea     (PLANE_A_MAP_LAYOUT).l,a0
		lea     ($E000).l,a1
		move.w  #$800,d0
		move.w  #2,d1
		jmp     (j_ApplyImmediateVramDma).l

    ; End of function sub_1316D8

byte_13171C:    dc.b 3
		dc.b   0
		dc.b   3
		dc.b   4
		dc.b   3
		dc.b   8
		dc.b   3
		dc.b  $C
		dc.b   3
		dc.b $30 
		dc.b   3
		dc.b $34 
		dc.b   3
		dc.b $38 
		dc.b   3
		dc.b $3C 
		dc.b   3
		dc.b $60 
		dc.b   3
		dc.b $64 
		dc.b   3
		dc.b $68 
		dc.b   3
		dc.b $6C 
		dc.b   3
		dc.b $90 
		dc.b   3
		dc.b $94 
		dc.b   3
		dc.b $98 
		dc.b   3
		dc.b $9C 
		dc.b   3
		dc.b $C0 
		dc.b   3
		dc.b $C4 
		dc.b   3
		dc.b $C8 
		dc.b   3
		dc.b $CC 
		dc.b   3
		dc.b $F0 
		dc.b   3
		dc.b $F4 
		dc.b   3
		dc.b $F8 
		dc.b   3
		dc.b $FC 
		dc.b   4
		dc.b $20
		dc.b   4
		dc.b $24 
		dc.b   4
		dc.b $28 
		dc.b   4
		dc.b $2C 
		dc.b   4
		dc.b $50 
		dc.b   4
		dc.b $54 
		dc.b   4
		dc.b $58 
		dc.b   4
		dc.b $5C 
		dc.b   3
		dc.b   1
		dc.b   3
		dc.b   5
		dc.b   3
		dc.b   9
		dc.b   3
		dc.b  $D
		dc.b   3
		dc.b $31 
		dc.b   3
		dc.b $35 
		dc.b   3
		dc.b $39 
		dc.b   3
		dc.b $3D 
		dc.b   3
		dc.b $61 
		dc.b   3
		dc.b $65 
		dc.b   3
		dc.b $69 
		dc.b   3
		dc.b $6D 
		dc.b   3
		dc.b $91 
		dc.b   3
		dc.b $95 
		dc.b   3
		dc.b $99 
		dc.b   3
		dc.b $9D 
		dc.b   3
		dc.b $C1 
		dc.b   3
		dc.b $C5 
		dc.b   3
		dc.b $C9 
		dc.b   3
		dc.b $CD 
		dc.b   3
		dc.b $F1 
		dc.b   3
		dc.b $F5 
		dc.b   3
		dc.b $F9 
		dc.b   3
		dc.b $FD 
		dc.b   4
		dc.b $21 
		dc.b   4
		dc.b $25 
		dc.b   4
		dc.b $29 
		dc.b   4
		dc.b $2D 
		dc.b   4
		dc.b $51 
		dc.b   4
		dc.b $55 
		dc.b   4
		dc.b $59 
		dc.b   4
		dc.b $5D 
		dc.b   3
		dc.b   2
		dc.b   3
		dc.b   6
		dc.b   3
		dc.b  $A
		dc.b   3
		dc.b  $E
		dc.b   3
		dc.b $32 
		dc.b   3
		dc.b $36 
		dc.b   3
		dc.b $3A 
		dc.b   3
		dc.b $3E 
		dc.b   3
		dc.b $62 
		dc.b   3
		dc.b $66 
		dc.b   3
		dc.b $6A 
		dc.b   3
		dc.b $6E 
		dc.b   3
		dc.b $92 
		dc.b   3
		dc.b $96 
		dc.b   3
		dc.b $9A 
		dc.b   3
		dc.b $9E 
		dc.b   3
		dc.b $C2 
		dc.b   3
		dc.b $C6 
		dc.b   3
		dc.b $CA 
		dc.b   3
		dc.b $CE 
		dc.b   3
		dc.b $F2 
		dc.b   3
		dc.b $F6 
		dc.b   3
		dc.b $FA 
		dc.b   3
		dc.b $FE 
		dc.b   4
		dc.b $22 
		dc.b   4
		dc.b $26 
		dc.b   4
		dc.b $2A 
		dc.b   4
		dc.b $2E 
		dc.b   4
		dc.b $52 
		dc.b   4
		dc.b $56 
		dc.b   4
		dc.b $5A 
		dc.b   4
		dc.b $5E 
		dc.b   3
		dc.b   3
		dc.b   3
		dc.b   7
		dc.b   3
		dc.b  $B
		dc.b   3
		dc.b  $F
		dc.b   3
		dc.b $33 
		dc.b   3
		dc.b $37 
		dc.b   3
		dc.b $3B 
		dc.b   3
		dc.b $3F 
		dc.b   3
		dc.b $63 
		dc.b   3
		dc.b $67 
		dc.b   3
		dc.b $6B 
		dc.b   3
		dc.b $6F 
		dc.b   3
		dc.b $93 
		dc.b   3
		dc.b $97 
		dc.b   3
		dc.b $9B 
		dc.b   3
		dc.b $9F 
		dc.b   3
		dc.b $C3 
		dc.b   3
		dc.b $C7 
		dc.b   3
		dc.b $CB 
		dc.b   3
		dc.b $CF 
		dc.b   3
		dc.b $F3 
		dc.b   3
		dc.b $F7 
		dc.b   3
		dc.b $FB 
		dc.b   3
		dc.b $FF
		dc.b   4
		dc.b $23 
		dc.b   4
		dc.b $27 
		dc.b   4
		dc.b $2B 
		dc.b   4
		dc.b $2F 
		dc.b   4
		dc.b $53 
		dc.b   4
		dc.b $57 
		dc.b   4
		dc.b $5B 
		dc.b   4
		dc.b $5F 
		dc.b   3
		dc.b $10
		dc.b   3
		dc.b $14
		dc.b   3
		dc.b $18
		dc.b   3
		dc.b $1C
		dc.b   3
		dc.b $40 
		dc.b   3
		dc.b $44 
		dc.b   3
		dc.b $48 
		dc.b   3
		dc.b $4C 
		dc.b   3
		dc.b $70 
		dc.b   3
		dc.b $74 
		dc.b   3
		dc.b $78 
		dc.b   3
		dc.b $7C 
		dc.b   3
		dc.b $A0 
		dc.b   3
		dc.b $A4 
		dc.b   3
		dc.b $A8 
		dc.b   3
		dc.b $AC 
		dc.b   3
		dc.b $D0 
		dc.b   3
		dc.b $D4 
		dc.b   3
		dc.b $D8 
		dc.b   3
		dc.b $DC 
		dc.b   4
		dc.b   0
		dc.b   4
		dc.b   4
		dc.b   4
		dc.b   8
		dc.b   4
		dc.b  $C
		dc.b   4
		dc.b $30 
		dc.b   4
		dc.b $34 
		dc.b   4
		dc.b $38 
		dc.b   4
		dc.b $3C 
		dc.b   4
		dc.b $60 
		dc.b   4
		dc.b $64 
		dc.b   4
		dc.b $68 
		dc.b   4
		dc.b $6C 
		dc.b   3
		dc.b $11
		dc.b   3
		dc.b $15
		dc.b   3
		dc.b $19
		dc.b   3
		dc.b $1D
		dc.b   3
		dc.b $41 
		dc.b   3
		dc.b $45 
		dc.b   3
		dc.b $49 
		dc.b   3
		dc.b $4D 
		dc.b   3
		dc.b $71 
		dc.b   3
		dc.b $75 
		dc.b   3
		dc.b $79 
		dc.b   3
		dc.b $7D 
		dc.b   3
		dc.b $A1 
		dc.b   3
		dc.b $A5 
		dc.b   3
		dc.b $A9 
		dc.b   3
		dc.b $AD 
		dc.b   3
		dc.b $D1 
		dc.b   3
		dc.b $D5 
		dc.b   3
		dc.b $D9 
		dc.b   3
		dc.b $DD 
		dc.b   4
		dc.b   1
		dc.b   4
		dc.b   5
		dc.b   4
		dc.b   9
		dc.b   4
		dc.b  $D
		dc.b   4
		dc.b $31 
		dc.b   4
		dc.b $35 
		dc.b   4
		dc.b $39 
		dc.b   4
		dc.b $3D 
		dc.b   4
		dc.b $61 
		dc.b   4
		dc.b $65 
		dc.b   4
		dc.b $69 
		dc.b   4
		dc.b $6D 
		dc.b   3
		dc.b $12
		dc.b   3
		dc.b $16
		dc.b   3
		dc.b $1A
		dc.b   3
		dc.b $1E
		dc.b   3
		dc.b $42 
		dc.b   3
		dc.b $46 
		dc.b   3
		dc.b $4A 
		dc.b   3
		dc.b $4E 
		dc.b   3
		dc.b $72 
		dc.b   3
		dc.b $76 
		dc.b   3
		dc.b $7A 
		dc.b   3
		dc.b $7E 
		dc.b   3
		dc.b $A2 
		dc.b   3
		dc.b $A6 
		dc.b   3
		dc.b $AA 
		dc.b   3
		dc.b $AE 
		dc.b   3
		dc.b $D2 
		dc.b   3
		dc.b $D6 
		dc.b   3
		dc.b $DA 
		dc.b   3
		dc.b $DE 
		dc.b   4
		dc.b   2
		dc.b   4
		dc.b   6
		dc.b   4
		dc.b  $A
		dc.b   4
		dc.b  $E
		dc.b   4
		dc.b $32 
		dc.b   4
		dc.b $36 
		dc.b   4
		dc.b $3A 
		dc.b   4
		dc.b $3E 
		dc.b   4
		dc.b $62 
		dc.b   4
		dc.b $66 
		dc.b   4
		dc.b $6A 
		dc.b   4
		dc.b $6E 
		dc.b   3
		dc.b $13
		dc.b   3
		dc.b $17
		dc.b   3
		dc.b $1B
		dc.b   3
		dc.b $1F
		dc.b   3
		dc.b $43 
		dc.b   3
		dc.b $47 
		dc.b   3
		dc.b $4B 
		dc.b   3
		dc.b $4F 
		dc.b   3
		dc.b $73 
		dc.b   3
		dc.b $77 
		dc.b   3
		dc.b $7B 
		dc.b   3
		dc.b $7F 
		dc.b   3
		dc.b $A3 
		dc.b   3
		dc.b $A7 
		dc.b   3
		dc.b $AB 
		dc.b   3
		dc.b $AF 
		dc.b   3
		dc.b $D3 
		dc.b   3
		dc.b $D7 
		dc.b   3
		dc.b $DB 
		dc.b   3
		dc.b $DF 
		dc.b   4
		dc.b   3
		dc.b   4
		dc.b   7
		dc.b   4
		dc.b  $B
		dc.b   4
		dc.b  $F
		dc.b   4
		dc.b $33 
		dc.b   4
		dc.b $37 
		dc.b   4
		dc.b $3B 
		dc.b   4
		dc.b $3F 
		dc.b   4
		dc.b $63 
		dc.b   4
		dc.b $67 
		dc.b   4
		dc.b $6B 
		dc.b   4
		dc.b $6F 
		dc.b   3
		dc.b $20
		dc.b   3
		dc.b $24 
		dc.b   3
		dc.b $28 
		dc.b   3
		dc.b $2C 
		dc.b   3
		dc.b $50 
		dc.b   3
		dc.b $54 
		dc.b   3
		dc.b $58 
		dc.b   3
		dc.b $5C 
		dc.b   3
		dc.b $80 
		dc.b   3
		dc.b $84 
		dc.b   3
		dc.b $88 
		dc.b   3
		dc.b $8C 
		dc.b   3
		dc.b $B0 
		dc.b   3
		dc.b $B4 
		dc.b   3
		dc.b $B8 
		dc.b   3
		dc.b $BC 
		dc.b   3
		dc.b $E0 
		dc.b   3
		dc.b $E4 
		dc.b   3
		dc.b $E8 
		dc.b   3
		dc.b $EC 
		dc.b   4
		dc.b $10
		dc.b   4
		dc.b $14
		dc.b   4
		dc.b $18
		dc.b   4
		dc.b $1C
		dc.b   4
		dc.b $40 
		dc.b   4
		dc.b $44 
		dc.b   4
		dc.b $48 
		dc.b   4
		dc.b $4C 
		dc.b   4
		dc.b $70 
		dc.b   4
		dc.b $74 
		dc.b   4
		dc.b $78 
		dc.b   4
		dc.b $7C 
		dc.b   3
		dc.b $21 
		dc.b   3
		dc.b $25 
		dc.b   3
		dc.b $29 
		dc.b   3
		dc.b $2D 
		dc.b   3
		dc.b $51 
		dc.b   3
		dc.b $55 
		dc.b   3
		dc.b $59 
		dc.b   3
		dc.b $5D 
		dc.b   3
		dc.b $81 
		dc.b   3
		dc.b $85 
		dc.b   3
		dc.b $89 
		dc.b   3
		dc.b $8D 
		dc.b   3
		dc.b $B1 
		dc.b   3
		dc.b $B5 
		dc.b   3
		dc.b $B9 
		dc.b   3
		dc.b $BD 
		dc.b   3
		dc.b $E1 
		dc.b   3
		dc.b $E5 
		dc.b   3
		dc.b $E9 
		dc.b   3
		dc.b $ED 
		dc.b   4
		dc.b $11
		dc.b   4
		dc.b $15
		dc.b   4
		dc.b $19
		dc.b   4
		dc.b $1D
		dc.b   4
		dc.b $41 
		dc.b   4
		dc.b $45 
		dc.b   4
		dc.b $49 
		dc.b   4
		dc.b $4D 
		dc.b   4
		dc.b $71 
		dc.b   4
		dc.b $75 
		dc.b   4
		dc.b $79 
		dc.b   4
		dc.b $7D 
		dc.b   3
		dc.b $22 
		dc.b   3
		dc.b $26 
		dc.b   3
		dc.b $2A 
		dc.b   3
		dc.b $2E 
		dc.b   3
		dc.b $52 
		dc.b   3
		dc.b $56 
		dc.b   3
		dc.b $5A 
		dc.b   3
		dc.b $5E 
		dc.b   3
		dc.b $82 
		dc.b   3
		dc.b $86 
		dc.b   3
		dc.b $8A 
		dc.b   3
		dc.b $8E 
		dc.b   3
		dc.b $B2 
		dc.b   3
		dc.b $B6 
		dc.b   3
		dc.b $BA 
		dc.b   3
		dc.b $BE 
		dc.b   3
		dc.b $E2 
		dc.b   3
		dc.b $E6 
		dc.b   3
		dc.b $EA 
		dc.b   3
		dc.b $EE 
		dc.b   4
		dc.b $12
		dc.b   4
		dc.b $16
		dc.b   4
		dc.b $1A
		dc.b   4
		dc.b $1E
		dc.b   4
		dc.b $42 
		dc.b   4
		dc.b $46 
		dc.b   4
		dc.b $4A 
		dc.b   4
		dc.b $4E 
		dc.b   4
		dc.b $72 
		dc.b   4
		dc.b $76 
		dc.b   4
		dc.b $7A 
		dc.b   4
		dc.b $7E 
		dc.b   3
		dc.b $23 
		dc.b   3
		dc.b $27 
		dc.b   3
		dc.b $2B 
		dc.b   3
		dc.b $2F 
		dc.b   3
		dc.b $53 
		dc.b   3
		dc.b $57 
		dc.b   3
		dc.b $5B 
		dc.b   3
		dc.b $5F 
		dc.b   3
		dc.b $83 
		dc.b   3
		dc.b $87 
		dc.b   3
		dc.b $8B 
		dc.b   3
		dc.b $8F 
		dc.b   3
		dc.b $B3 
		dc.b   3
		dc.b $B7 
		dc.b   3
		dc.b $BB 
		dc.b   3
		dc.b $BF 
		dc.b   3
		dc.b $E3 
		dc.b   3
		dc.b $E7 
		dc.b   3
		dc.b $EB 
		dc.b   3
		dc.b $EF 
		dc.b   4
		dc.b $13
		dc.b   4
		dc.b $17
		dc.b   4
		dc.b $1B
		dc.b   4
		dc.b $1F
		dc.b   4
		dc.b $43 
		dc.b   4
		dc.b $47 
		dc.b   4
		dc.b $4B 
		dc.b   4
		dc.b $4F 
		dc.b   4
		dc.b $73 
		dc.b   4
		dc.b $77 
		dc.b   4
		dc.b $7B 
		dc.b   4
		dc.b $7F 
